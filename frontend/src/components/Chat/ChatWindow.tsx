...
export default function ChatWindow({ conversation, onClose, onMinimize, isMinimized }: ChatWindowProps) {
  const { primaryAccentColor, secondaryAccentColor, isDarkMode } = useThemeStore();
  const { getMessages, sendMessage: apiSendMessage, markMessagesAsRead, user } = useAuthStore();
  const { socket, connect, on: socketOn, off: socketOff, sendMessage: socketSendMessage } = useChatSocketStore();
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      // Send via API first
      const result = await apiSendMessage(conversation.otherUser._id, newMessage.trim());
      ...
      <div className={`flex items-center justify-between p-3 border-b border-gray-200 ${
        ...
        <div className="flex items-center gap-3 flex-1 min-w-0" onClick={onMinimize}>
          ...
          <div className="flex-1 min-w-0">
            <div className={`font-medium text-sm truncate ${isDarkMode ? 'text-white' : 'text-gray-800'}`}>
        ...
        <div className="flex items-center gap-2">
          ...
          >
            <FaMinus className={isDarkMode ? 'text-gray-300' : 'text-gray-600'} size={12} />
          </button>
          <button
            onClick={onClose}
            className={`p-1 rounded hover:bg-gray-200 ${isDarkMode ? 'hover:bg-gray-600' : ''}`}
          >
            <FaTimes className={isDarkMode ? 'text-gray-300' : 'text-gray-600'} size={12} />
          </button>
        </div>
      </div>
